    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable"  @click="shelf(props,'false')">下架</v-btn>
          <v-btn icon v-else @click="shelf(props,'true')">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      shelf(props,flag){
        this.$http.put("/item/spu/shelf", {"id":props.item.id,"saleable":flag}).then(resp => {
          this.goodsList.splice(props.index, 1);
          if(flag == true){
